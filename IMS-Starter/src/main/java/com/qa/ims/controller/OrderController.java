	...
	public Orders create() {
		...
		while (order == null) {
			...
			while (order1 == null) {
				...
				order = orderDAO.create(new Orders(customerID, itemID, quantity));
		...
		while (order == null) {
			...
			while (order1 == null) {
				...
				while (order2 == null) {
					...
					Long quantity = utils.getLong();
					if (quantity == 0) {
						LOGGER.info("Returned To Setting ItemID");
						continue;
					}
					if (quantity == -1) {
						LOGGER.info("REQUEST CANCELLED");
					...
					order = orderDAO.update(new Orders(orderID, customerID, itemID, quantity, 0d));
